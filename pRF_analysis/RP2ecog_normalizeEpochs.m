function [epochsnorm] = RP2ecog_normalizeEpochs(epochs, t, baselineTime, baselineType, idx)
...
epochsnorm = zeros(size(epochs, 1),size(epochs,2), size(epochs,3));
% normalize
for ii = 1:length(runs)
    ...
    switch baselineType
        case 'percentsignalchange'
            ...
            %epochsnorm(:,idx_i,:) = epochs(:,idx_i,:)./m_base-1;
            m_base     = squeeze(mean(mean(epochs(:,idx_i,base_range),3, 'omitnan'), 2, 'omitnan'));
            epochsnorm(:,:,:) = epochs(:,:,:)./abs(m_base)-1;
            end
...
epochsnorm = permute(epochsnorm, [3 2 1]);
