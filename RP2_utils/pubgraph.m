...
function pubgraph(fh,FS,LW,CL,xax)
...
%format X and Y axis tickmarks so all numbers are displayed with the same precision
    ...
    YTick = str2double(string(get(ax,'YTickLabel')));          %get the current Y axis labels
    if xax
        XTick = str2double(string(get(ax,'XTickLabel')));          %get the current X axis labels
    else
        XTick = 0;
    end
